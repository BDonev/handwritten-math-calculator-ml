...
def get_output_image(path):
    ...
    ret3, thresh = cv2.threshold(blur, 95, 255, cv2.THRESH_BINARY) #+ cv2.THRESH_OTSU)
    ...
    for j, cnt in enumerate(contours):
        ...
        if (hierarchy[0][j][3] != -1 and w > 10 and h > 10):
            ...
            cv2.rectangle(img_org, (x, y), (x + w + 5, y + h + 5), (0, 255, 0), 2)

            # cropping each image and process
            roi = img[y:y + h + 5, x:x + w + 5]
            roi = cv2.bitwise_not(roi)
            roi = image_refiner(roi)
            cv2.threshold(roi, 50, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
...
label_map = np.load('label_map.npy').item()
output_img = get_output_image("photo_1.jpg")
